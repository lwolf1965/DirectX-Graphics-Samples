...
)
{
    ...
    uint2 flags;
    float unusedT;
    BoundingBox topLevelBox = GetLeftBoxFromBVH(
        currentBVH,
        0,
        flags);
    ...
    while (nodesToProcess[TOP_LEVEL_INDEX] != 0)
    {
        ...
        do
        {
            ...
            uint2 flags;
            BoundingBox box = GetLeftBoxFromBVH(
                currentBVH,
                thisNodeIndex,
                flags);

            {
                ...
                {
                    MARK(9, 0);
                    const uint leftChildIndex = flags.x;
                    const uint rightChildIndex = flags.x;

                    float resultT = RayTCurrent();
                    uint2 flags;
                    float leftT, rightT;
                    BoundingBox leftBox = GetLeftBoxFromBVH(
                        currentBVH,
                        leftChildIndex,
                        flags);

                    BoundingBox rightBox = GetLeftBoxFromBVH(
                        currentBVH,
                        rightChildIndex,
                        flags);
